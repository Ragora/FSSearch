...
class Flags(Token):
    """
        A flag token is the initial leading meta data on the query string. Flags are utilized to
        do things such as turning on query timing, debugging outputs and so on.
    """

    __PATTERN__ = re.compile("([A-z]+(,[A-z]+)*):")
    """
        The pattern associated with this token.
    """
